# Content of Landing Page

# GetLanded Website Content Documentation

This document contains a comprehensive, verbatim record of all text content visible on the GetLanded website front-end. Every title, description, button label, and duplicate text is included to ensure a complete “10/10” reference.

---

## **1. Global Header**

*Component: `src/components/layout/Header.tsx`*

- **Logo Text:** GetLanded
- **Navigation Links:**
    - Home
    - Product
    - Pricing
    - About
    - Blog
- **Navbar Buttons:**
    - Get Started

---

## **2. Home Page**

*Page: `src/pages/Home.tsx`*

### **2.1. Hero Section**

*Component: `src/components/home/Hero.tsx`*

- **Main Headline:** GetLanded – Your AI Career Co-Pilot
- **Sub-headline:** The all-in-one platform to track applications, optimize resumes, and land your dream job 3x faster.
- **Primary CTA Button:** Get Started Free
- **Secondary Text:**
    - No credit card required
    - Free forever plan

### **2.2. Value Proposition**

*Component: `src/components/home/ValueProp.tsx`*

- **Headline:** Your all-in-one solution for the modern job search
- **Description:** Stop juggling spreadsheets and generic advice. Get the tools you need to stand out and get hired.

### **2.3. Stats Section**

*Component: `src/components/home/Stats.tsx`*

- **Section Title:** Proven Results
- **Main Headline:** Receipts? We’ve got plenty.
- **Card 1 (Offers):**
    - **Title:** 500+ Offers Landed
    - **Description:** From top tech companies to specialized startups.
- **Card 2 (Interviews):**
    - **Title:** 89% Interview Rate
    - **Description:** For users with optimized profiles.

### **2.4. Features Section**

*Component: `src/components/home/Features.tsx`*

- **Section Headline:** Everything you need to get hired
- **Feature Cards:**
    1. **Title:** Smart Job Match Extension
        - **Description:** Instant fit score and keyword analysis for any job posting.
    2. **Title:** AI Resume Builder
        - **Description:** Tailor your resume for every application in seconds.
    3. **Title:** Visa Sponsorship Analyzer
        - **Description:** Instantly see H1B history and green card policies for over 500k companies.
    4. **Title:** AI Mock Interviews
        - **Description:** Practice with realistic questions tailored to your target role.
    5. **Title:** Progress Tracking
        - **Description:** Visualize your job search funnel and identify bottlenecks.
- **Hero Feature (Bottom):**
    - **Title:** Track every application in one place
    - **Description:** Automatically save jobs from LinkedIn, Indeed, and more with a single click. Organize your search with a powerful Kanban board.
    - **CTA Button:** Start Tracking Free

### **2.5. How It Works**

*Component: `src/components/home/HowItWorks.tsx`*

- **Section Title:** How it works
- **Headline:** Your path to a better job
- **Description:** Three simple steps to transform your job search.
- **Step 1:**
    - **Title:** Install the extension
    - **Description:** Add GetLanded to Chrome to save jobs and analyze listings instantly.
- **Step 2:**
    - **Title:** AI analyzes every role
    - **Description:** Get instant match scores and keyword gaps based on your profile.
- **Step 3:**
    - **Title:** Apply smarter
    - **Description:** Use AI-tailored resumes and tracking to land more interviews.

### **2.6. AI Tools Section**

*Component: `src/components/home/ToolsSection.tsx`*

- **Headline:** AI-Powered Tools
- **Tool Cards:**
    1. **Title:** LinkedIn Profile Optimizer
        - **Description:** Stand out to recruiters with AI-enhanced headlines and summaries.
    2. **Title:** Chrome Extension
        - **Description:** Save jobs and get insights while you browse.
    3. **Title:** Interview Prep
        - **Description:** Practice questions specific to the company and role.
    4. **Title:** Cover Letter Generator
        - **Description:** Write personalized cover letters in seconds.
    5. **Title:** Resume Scanner
        - **Description:** Check your resume against ATS systems.
    6. **Title:** Salary Insights
        - **Description:** Real-time compensation data for your target roles.

### **2.7. FAQ Section**

*Component: `src/pages/Home.tsx` (Inline Data)*

- **Headline:** Frequently Asked Questions
- **Q1:** How does the AI Resume Optimization work?
    - **A1:** Our AI analyzes your projects and experiences, then rewrites them into recruiter-friendly bullet points that match the specific job description you represent.
- **Q2:** Is there a free version?
    - **A2:** Yes! Our Free Beta plan includes 25 job saves and basic resume scanning.
- **Q3:** Does it work for visa sponsorship jobs?
    - **A3:** Absolutely. We specialize in helping international students find H1B and sponsorship-friendly roles.
- **Q4:** Which job boards do you support?
    - **A4:** Currently we support LinkedIn, Indeed, Glassdoor, and 40+ other major job sites.

---

## **3. Product Page**

*Page: `src/pages/Product.tsx`*

### **3.1. Hero Section**

- **Headline:** The AI Co-Pilot for your Job Search
- **Sub-headline:** Stop applying blindly. Get real-time insights, auto-tailored resumes, and organized tracking—all in one place.
- **Primary CTA Button:** Add to Chrome - It’s Free
- **Secondary CTA Button:** Watch Demo

### **3.2. Explore Features (Grid)**

- **Headline:** Explore Features
- **Categories:**
    1. **Title:** Resume Builder
        - **Subtitle:** AI-Powered Tailoring
    2. **Title:** Job Application Tracker
        - **Subtitle:** Kanban Board
    3. **Title:** Networking Tracker
        - **Subtitle:** Manage Connections
    4. **Title:** LinkedIn Optimizer
        - **Subtitle:** Profile Enhancement
    5. **Title:** AI Mock Interview
        - **Subtitle:** Practice & Feedback

### **3.3. Detailed Feature Sections**

- **Feature 1 (Resume Builder):**
    - **Badge:** Resume Builder
    - **Headline:** Quickly tailor your resume for each job with AI
    - **Description:** Use the right keywords from the job description to highlight your best experience on your resume. Get recommendations to improve your resume and land more interviews.
    - **Button:** Build a Resume with AI
    - **Visual Text:** “WeWork”, “32%”, “Tailor your resume to increase match score”, “True Match”
- **Feature 2 (Job Tracker):**
    - **Badge:** Job Tracker
    - **Headline:** One place to organize and manage your job search
    - **Description:** A free and easy way to keep track of all your job applications in one place. Bookmark jobs from any job board using our Chrome extension to create your master list.
    - **Button:** Start Organizing Now
    - **Visual Text:** “Job Position”, “Company”, “Location”, “Status”, “APPLIED”, “SCREEN”, “INTERVIEW”
- **Feature 3 (Job Insights):**
    - **Badge:** Job Insights
    - **Headline:** Companies have recruiters, and you have GetLanded
    - **Description:** Save time by discovering jobs perfectly matched to your profile and resume. Filter results based on company, location, visa sponsorship and more.
    - **Button:** Start Getting Insights
    - **Visual Text:** “Marketing Manager, Commerce Services”, “ADOBE • Austin, TX”, “$131,000 - $170,000”, “Jobs in profile”, “Sponsorship”, “Full-time”, “Hard Skills”
- **Feature 4 (Bookmark Jobs):**
    - **Badge:** Bookmark Jobs
    - **Headline:** Save Jobs From over 40 Popular Job Boards
    - **Description:** Use the GetLanded Chrome extension to bookmark job postings from LinkedIn, Indeed, Glassdoor and dozens more. Seamlessly transition your job search from browser to tracker.
    - **Button:** Bookmark Jobs Now
    - **Visual Text:** “getlanded”, “Data Scientist - Product”, “by Toyota (View all jobs)”, “San Francisco, CA • New York, NY • Seattle, WA”, “$135,000 - $260,000”

### **3.4. Coming Soon**

- **Headline:** Coming soon
- **Description:** These features are in development and will be released after the initial beta launch.
- **Card 1:**
    - **Title:** AI Mock Interviews
    - **Desc:** Practice with AI-powered feedback and company-specific questions.
- **Card 2:**
    - **Title:** AI Interview Prep
    - **Desc:** Comprehensive guides and resources.
- **Card 3:**
    - **Title:** AI Humanizer
    - **Desc:** Make AI-generated text sound more natural.
- **Card 4:**
    - **Title:** Advanced Resume Rewriting
    - **Desc:** Deep restructuring of resume content.
- **Card 5:**
    - **Title:** Advanced Job Matching
    - **Desc:** Smarter algorithms for better job recommendations.
- **Card 6:**
    - **Title:** Plagiarism Checker
    - **Desc:** Ensure your content is unique.

### **3.5. Bottom CTA**

- **Badge:** Join 2.3M+ students
- **Headline:** Start your job search with clarity
- **Description:** Join our early-access waitlist and be among the first to experience AI-powered job search.
- **Button:** Join Waitlist

### **3.6. Product FAQ**

- **Headline:** Frequently Asked Questions
- **Subtitle:** Everything you need to know about GetLanded.
- *Uses same FAQ questions as Home Page.*

---

## **4. Pricing Page**

*Page: `src/pages/Pricing.tsx`*

### **4.1. Header**

- **Headline:** Simple, transparent pricing
- **Sub-headline:** Start for free, upgrade when you need more power. No hidden fees.
- **Toggle:** Monthly / Yearly

### **4.2. Pricing Plans**

- **Plan 1: Free Beta**
    - **Price:** $0/mo (Monthly & Yearly)
    - **Description:** Perfect for trying things out and getting started.
    - **Features:**
        - Track up to 25 applications
        - Basic Resume Scanner (5/mo)
        - Chrome Extension Access
        - Standard Support
    - **CTA Button:** Get Started Free
- **Plan 2: Pro**
    - **Price:** $12/mo (Monthly) / $9/mo (Yearly)
    - **Badge:** Most Popular
    - **Description:** For serious job seekers who want to land roles faster.
    - **Features:**
        - Unlimited Application Tracking
        - AI Resume Tailoring (50/mo)
        - Visa Sponsorship Insights
        - LinkedIn Profile Optimizer
        - Priority Support
    - **CTA Button:** Get Started
- **Plan 3: Pro Plus**
    - **Price:** $29/mo (Monthly) / $24/mo (Yearly)
    - **Description:** Ultimate power for maximizing your career potential.
    - **Features:**
        - Everything in Pro
        - Unlimited AI Resume Tailoring
        - Mock Interview Practice (Unlimited)
        - Cover Letter Generator
        - 1-on-1 Career Coaching session
    - **CTA Button:** Get Started

### **4.3. Testimonials**

- **Section Title:** Trusted by students and professionals from top universities and companies
- **Testimonials Data:**
    - “GetLanded helped me track 47 applications and land 3 offers. The resume scoring is a game changer.” (Sarah Chen, Software Engineer, Google, Stanford ’23)
    - “Detailed information about visa sponsorship saved me so much time. Highly recommend for international students.” (Rahul Patel, Product Manager, Microsoft, Georgia Tech ’22)
    - “The Chrome extension makes saving jobs from LinkedIn incredibly fast. I love the kanban board.” (Emily Rodriguez, UX Designer, Spotify, RISD ’23)
    - “Finally, a tool that actually helps with the job search rather than just listing jobs. The AI feedback is spot on.” (David Kim, Data Scientist, Netflix, UC Berkeley ’22)
    - “I landed my dream internship in 3 weeks using the networking tracker. It kept me accountable.” (Priya Sharma, Business Analyst, Deloitte, NYU ’24)

### **4.4. Pricing FAQ**

- *Q:* How does the AI Resume Optimization work?
    - *A:* Our AI analyzes your projects and experiences, then rewrites them into recruiter-friendly bullet points that match the specific job description you represent.
- *Q:* Is there a free version?
    - *A:* Yes! Our Free Beta plan includes 25 job saves and basic resume scanning.
- *Q:* Does it work for visa sponsorship jobs?
    - *A:* Absolutely. We specialize in helping international students find H1B and sponsorship-friendly roles.
- *Q:* Which job boards do you support?
    - *A:* Currently we support LinkedIn, Indeed, Glassdoor, and 40+ other major job sites.
- *Q:* Can I cancel anytime?
    - *A:* Yes, you can cancel your subscription at any time from your account settings. You will retain access until the end of your billing period.

### **4.5. Bottom CTA**

- **Headline:** Ready to get hired?
- **Description:** Join thousands of job seekers who are landing their dream roles faster with GetLanded.
- **Primary CTA:** Get Started Now (+ Arrow icon)
- **Secondary CTA:** View Demo
- **Trust Indicators:** No credit card needed, 14-day free pro trial

---

## **5. About Page**

*Page: `src/pages/About.tsx`*

### **5.1. Hero Section**

- **Headline:** Empowering every student to launch their dream career.
- **Sub-headline:** We’re building the co-pilot for the modern job search—transparent, intelligent, and fair.

### **5.2. Origin Story**

- **Visual Text:** Built in Liverpool, UK
- **Headline:** It started with a simple problem.
- **Text:** Applying to jobs was a mess of spreadsheets, 50 open tabs, and forgotten passwords. We wanted a way to save jobs from anywhere and see real insights instantly.

### **5.3. Timeline**

- **Stage 1: The Spark**
    - **Description:** A minimalist browser extension to help students save jobs from any site.
- **Stage 2: The Expansion**
    - **Description:** Adding visa sponsorship insights to level the playing field.
- **Stage 3: The Co-Pilot**
    - **Description:** A full AI platform to guide candidates from application to offer.

### **5.4. Manifesto**

- **Headline:** The job search is broken. We’re fixing it.
- **Text:** We believe that finding a job shouldn’t be a full-time job. The current system is opaque, inefficient, and often unfair. We’re using AI not to replace the human element, but to empower candidates with the insights and tools they need to tell their story effectively.

### **5.5. Core Pillars**

- **Pillar 1: Fairness & Transparency**
    - **Description:** We prioritize equal opportunity by clearly indicating visa sponsorship options and avoiding hidden algorithms. Our tools are built to foster trust and clarity in the hiring process.
- **Pillar 2: Innovation**
    - **Description:** We’re constantly pushing the boundaries of what’s possible with AI to give candidates a competitive edge.
- **Pillar 3: Privacy & Security**
    - **Description:** Your career data is personal. We treat it with the highest level of security and never sell your data.
- **Pillar 4: Empathy**
    - **Description:** We understand the stress of job hunting. Every feature we build is designed to reduce anxiety.
- **Pillar 5: True Clarity**
    - **Description:** Cutting through the noise to give you actionable insights and clear next steps.
- **Pillar 6: Global Reach**
    - **Description:** Helping talent cross borders. We specialize in international career mobility.

### **5.6. Team**

- **Headline:** The Collective
- **Text:** GetLanded is a diverse collective of software engineers, data scientists, and career coaches. Our founding team has experience at top tech companies and recruitment agencies. We partner closely with immigration specialists to ensure our visa checks remain up to date.
- **Member:** Nivin Vincent Raj
    - **Role:** Founder & CEO
    - **Expertise:** Software Engineering & Product Vision
    - **Visual Text:** Expertise

### **5.7. Footer CTA**

- **Headline:** Ready to land your dream job?
- **Description:** Everything you need to organize, optimize, and track your applications. In one place.
- **Button:** Get Started for Free

---

## **6. Blog Page**

*Page: `src/pages/Blog.tsx`*

- **Header:**
    - **Headline:** Career Insights
    - **Sub-headline:** Expert advice to help you navigate your professional journey.
- **Categories:** All, Interview Tips, Resume, Networking, Success Stories
- **Featured Article:**
    - **Title:** The complete guide to landing a job at FAANG companies.
    - **Category:** Featured
    - **Detail:** July 1, 2025 • 8 min read
    - **Description:** Discover the step-by-step process, from resume screening to the final onsite loop. We break down what recruiters are truly looking for in top candidates.
    - **Action:** Read Article
- **Articles Grid:**
    1. **Title:** How to ace your system design interview
        - **Detail:** Interview Tips • Jun 20, 2025 • 12 min read
    2. **Title:** 10 resume mistakes to avoid in 2025
        - **Detail:** Resume • Jun 18, 2025 • 5 min read
    3. **Title:** Networking strategies for introverts
        - **Detail:** Networking • Jun 15, 2025 • 6 min read
    4. **Title:** How I went from Junior to Senior in 2 years
        - **Detail:** Success Stories • Jun 12, 2025 • 4 min read
    5. **Title:** Negotiating your salary like a pro
        - **Detail:** Interview Tips • Jun 10, 2025 • 7 min read
    6. **Title:** Building a personal brand on LinkedIn
        - **Detail:** Networking • Jun 08, 2025 • 9 min read
- **Bottom Button:** Load More Insights

---

## **7. Dashboard (Mock Interface)**

*Page: `src/pages/Dashboard.tsx`*

- **Sidebar:**
    - **Logo:** GetLanded
    - **Nav Items:** Overview, Jobs, Resumes, Messages, Settings
    - **Promo:** Pro Plan - You have 5 premium scans remaining. -> Upgrade Now
- **Top Bar:**
    - **Search:** Search jobs, companies…
- **Main Content:**
    - **Greeting:** Welcome back, Alex
    - **Sub-greeting:** Here’s what’s happening with your job search today.
    - **Primary Button:** Find New Jobs
    - **Stats:**
        - Total Applications: 45 (+12%)
        - Interviews Scheduled: 3 (+1)
        - Profile Views: 128 (+24%)
        - Response Rate: 18% (+2%)
    - **Recent Applications:**
        - Spotify - Product Designer - Interview - 2h ago
        - Airbnb - Frontend Developer - Applied - 1d ago
        - Linear - Software Engineer - Rejected - 3d ago
        - Notion - Growth Engineer - Offer - 5d ago
        - Figma - Product Manager - Applied - 1w ago
        - **Link:** View All
    - **Right Column:**
        - **Weekly Goal:** 12/15 - Applications sent this week. Keep it up!
        - **Upcoming Events:**
            - Oct 24 - Spotify Interview - Product Design Round - 2:00 PM
            - Oct 26 - Resume Review - With Career Coach - 10:00 AM

---

## **8. Resources Page**

*Page: `src/pages/Resources.tsx`*

- **Headline:** Free Resources
- **Sub-headline:** Tools, templates, and guides to accelerate your job search.
- **Resource Cards:**
    1. **Title:** Modern Resume Template
        - **Type:** Template
        - **Desc:** Clean, ATS-friendly resume layout.
    2. **Title:** The Perfect Cover Letter
        - **Type:** Guide
        - **Desc:** Step-by-step writing guide.
    3. **Title:** Interview Masterclass
        - **Type:** Video
        - **Desc:** 1 hour video course on behavioral answers.
    4. **Title:** LinkedIn Profile Optimization
        - **Type:** Checklist
        - **Desc:** 20-point checklist.
    5. **Title:** Resignation Letter
        - **Type:** Template
        - **Desc:** Professional templates for leaving.
    6. **Title:** Salary Estimator
        - **Type:** Link
        - **Desc:** Calculator for market rates.
- **Newsletter Section:**
    - **Headline:** Subscribe to our newsletter
    - **Sub-headline:** Get the latest career tips delivered to your inbox.
    - **Placeholder:** Enter your email
    - **Button:** Subscribe

---

## **9. Support Page**

*Page: `src/pages/Support.tsx`*

- **Headline:** How can we help?
- **Search Placeholder:** Search for answers…
- **Quick Links:**
    1. **Title:** Getting Started
        - **Desc:** Guide to setting up your profile
    2. **Title:** Billing & Plans
        - **Desc:** Manage your subscription
    3. **Title:** Account Settings
        - **Desc:** Update email & password
- **FAQ:**
    - **Headline:** Frequently Asked Questions
    - **Questions:**
        - How do I import my resume from LinkedIn?
        - Can I cancel my subscription at any time?
        - Is my data private and secure?
        - Do you offer refunds?
        - How do I contact a career coach?
- **Contact Form:**
    - **Headline:** Still need help?
    - **Labels:** Name, Email, Message
    - **Placeholders:** John Doe, john@example.com, Describe your issue…
    - **Button:** Send Message

---

## **10. Waitlist Page**

*Page: `src/pages/Waitlist.tsx`*

- **Visual:** “G” Logo
- **Headline:** Join the Inner Circle
- **Sub-headline:** Get early access to our new AI Coaching features. Limited spots available for the beta program.
- **Placeholder:** Enter your email address
- **Button:** Join Waitlist
- **Social Proof:** 1,402 people joined this week

---

## **11. Global Footer**

*Component: `src/components/layout/Footer.tsx`*

- **Product Links:**
    - Features
    - Pricing
    - Waitlist
- **Company Links:**
    - About
    - Blog
    - Resources
    - Support
- **Legal Links:**
    - Privacy
    - Terms
- **Newsletter:**
    - **Headline:** Subscribe to our newsletter
    - **Description:** The latest career tips and resources, sent to your inbox weekly.
    - **Placeholder:** Enter your email
    - **Button:** Subscribe
- **Copyright:** © 2025 GetLanded. All rights reserved.